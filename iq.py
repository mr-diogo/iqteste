...
API = IQ_Option('login','senha') # Coloca o login e senha
...
def cataloga(par, dias, prct_call, prct_put, timeframe):
    ...
    analise = {}
    for velas in data:
        horario = datetime.fromtimestamp(velas['from']).strftime('%Y-%m-%d')
...
print('\n\nQual time frame deseja catalogar?:', end='')
timeframe = int(input())
...
print('\nTestar com quantos martim gales?:', end='')
martingale = input()
...
for par in catalogacao:
    for horario in sorted(catalogacao[par]):
        ...
        if ok == True:
            ...
            print(msg)
            open('sinais_'+str((datetime.now()).strftime('%m/') + ' - ' + str(timeframe) + 'M.txt', 'a').write(horario + ',' + par + ',' + catalogacao[par][horario]['dir'].strip()))
